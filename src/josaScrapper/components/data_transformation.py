...
class DataTransformationComponent:
    ...
    async def initiate(self)->DataIngestionArtifact:
        ...
        logging.info(f"Creating directory at {self.data_transformation_config.data_ingestion_dir} for data ingestion")
        os.makedirs(self.data_transformation_config.data_ingestion_dir, exist_ok=True)

        logging.info(f"Creating directory at {os.path.dirname(self.data_transformation_config.feature_store_file_path)} for feature store")
        dir_name=os.path.dirname(self.data_transformation_config.feature_store_file_path)
        os.makedirs(dir_name, exist_ok=True)

        logging.info(f"Fetching data from s3")
        data:pd.DataFrame=await self.data_access_class.make_data()
        data.to_csv(self.data_transformation_config.feature_store_file_path, index=False)


        logging.info(f"Splitting data into train and test sets with ratio {self.data_transformation_config.train_test_split_ratio}")
        ...
        train_set.to_csv(self.data_transformation_config.training_file_path, index=False)
        test_set.to_csv(self.data_transformation_config.testing_file_path, index=False)
        data_ingestion_artifact:DataIngestionArtifact=DataIngestionArtifact(
            trained_file_path=self.data_transformation_config.training_file_path,
            test_file_path=self.data_transformation_config.testing_file_path
        )
